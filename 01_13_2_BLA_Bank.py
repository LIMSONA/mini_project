        ...
        if inputaccount in allAccount: # 계좌번호가 존재하면 금액 입력
            ...
            for account in bankinform: # 계좌번호 찾기 
                if inputaccount == account[0]: # 
                    ...
                    print("\n* {}원 입금이 완료 되었습니다. 첫화면으로 이동합니다.".format(inputmoney))            
                    BankSystem.pyHistoryD()
                    BankSystem.pyUpdate()    
            # 입금 내역 기록
            ...
            for account in bankinform:
                if inputaccount == account[0]:
                    if int(account[2]) >= inputmoney:
                        ...
                        print("\n* {}원 출금이 완료 되었습니다. 첫화면으로 이동합니다.".format(inputmoney))
                        BankSystem.pyHistoryW()
                        BankSystem.pyUpdate()            
...
class BankSystem:
    def pyUpdate():
        ...
        set 계좌번호=%s,이름=%s,잔액=%s
        where 계좌번호=%s;
        '''
        # for data in bankinform:                      
        #     cur.execute(sql,(str(data[0]),str(data[1]),int(data[2]),str(data[0])))
        cur.execute(sql,("9999","ㅇㅇㅇ",100000000,"9999"))
        conn.commit()
        cur.close()
        conn.close()
    ...
    def pyHistoryD():
        ...
        sql =''' 
        insert into history(계좌번호,이름,입출금,금액)
        values(%s,%s,%s,%s);
        '''
        for data in bankinform:                      
            cur.execute(sql,(str(data[0]),str(data[1]),"입금",int(data[2])))
        conn.commit()
        cur.close()
        conn.close()
    ...
    def pyHistoryW():
        ...
        '''
        for data in bankinform:                      
            cur.execute(sql,(str(data[0]),str(data[1]),"출금",int(data[2])))
        conn.commit()
        cur.close()
        conn.close()
