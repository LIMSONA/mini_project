...
class AccountCreate:
    ...
    def __init__(self, userAccount, userName, userBalance=0):
        ...
        BankSystem.pyHistoryD(userBalance)
                ...
                for account in bankinform:
                    if inputaccount == account[0]:
                        if int(account[2]) >= inputmoney:
                            ...
                            BankSystem.pyHistoryW(inputmoney)
...
if __name__ == "__main__":
    if os.path.isfile('bankaccountlist.txt'): # bankaccountlist 텍스트파일 찾기
        with open('bankaccountlist.txt', 'r') as f:
            datas = f.readlines()

            # bankinform에는 실제 [[계좌,이름,금액],[계좌,이름,금액]]형식으로 들어가있음          
            for data in datas: #위의 datas = f.readlines()
                bankinform.append(data[:-1].split(":")) #줄바꿈 표시 제외하기위해서 -1까지
